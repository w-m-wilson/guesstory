...
import { sanitizeStr, sanitizeItems } from './_sanitize.js'
...
export default async function handler(req, res) {
  ...
  const safeGuess = sanitizeStr(guess)
  const safeCategory = sanitizeStr(category)
  const safeItems = foundItems ? sanitizeItems(foundItems) : []
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      max_tokens: 80,
      system:
        `You are a guide in "Guesstory", a ranking puzzle game. The hidden category is: "${safeCategory}".\n` +
        ...
        `Items found so far: ${safeItems.length ? safeItems.join(', ') : 'none yet'}\n\n` +
        `The player typed "${safeGuess}" and got a miss. Write ONE short actionable hint (max 20 words):\n` +
      ...
      messages: [{ role: 'user', content: `Player typed: "${safeGuess}"` }],
    })
